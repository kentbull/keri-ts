...
import { Baser, BaserOptions } from '../../../src/db/basing.ts';
  ...
  it("should test Baser.getAllEvtsIter directly", async () => {
    await run(function* () {
      ...
      try {
        ...
        console.log('Is generator?', iter.constructor.name === 'GeneratorFunction' || iter[Symbol.iterator]);
        ...
        try {
          for (const [keyBytes, valBytes] of iter) {
            entryCount++;
            console.log(`Entry ${entryCount}: key length=${keyBytes.length}, val length=${valBytes.length}`);
          ...
          console.error('Error type:', iterError instanceof Error ? iterError.constructor.name : typeof iterError);
          console.error('Error message:', iterError instanceof Error ? iterError.message : String(iterError));
          console.error('Error stack:', iterError instanceof Error ? iterError.stack : 'No stack trace');
          throw iterError;
        }
      } finally {
        yield* baser.close();
