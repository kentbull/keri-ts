...
| `subing.py`    | `class`     | `CryptSignerSuber`  |        1781 | `DB-SUBER`  | `keripy/src/keri/db/subing.py`    | `packages/keri/src/db/subing.ts`                | `CryptSignerSuber`        | `D2`  | `Implemented` | Sealed-box encrypt/decrypt semantics are now landed through the KERI-local libsodium backend and exercised by focused `pris.` round-trip tests plus Gate D reopen coverage                                                                               |
...
| `basing.py`    | `class`     | `Baser`             |         603 | `DB-BASER`  | `keripy/src/keri/db/basing.py`    | `packages/keri/src/db/basing.ts`                | `Baser`                   | `D4`  | `Partial`     | Current TS `Baser` binds broad named-subdb surface needed for local runtime arc, including event/state backbone, reply/OOBI/exc/contact families, and compat-mode `.keri/db` support; escrow/process-loop closure and full row-by-row parity remain open |
