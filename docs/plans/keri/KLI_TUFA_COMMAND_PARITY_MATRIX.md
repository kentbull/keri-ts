...
- `Harness-Ready`: modeled in harness but waiting on command implementation.
...
| `B`  | `B-LIST-BEFORE`         | `kli list`                        | `tufa list`                           | Empty identifier set before first incept             | `Blocked`       | `packages/keri/test/integration/app/interop-gates-harness.test.ts` |
| `B`  | `B-LIST-AFTER`          | `kli list`                        | `tufa list`                           | Includes `alias (prefix)` after incept               | `Blocked`       | `packages/keri/test/integration/app/interop-gates-harness.test.ts` |
| `B`  | `B-AID`                 | `kli aid --alias <a>`             | `tufa aid --alias <a>`                | Returns same prefix as `incept`                      | `Blocked`       | `packages/keri/test/integration/app/interop-gates-harness.test.ts` |
| `C`  | `C-KLI-STORE-OPEN`      | `kli init/incept` then `kli list` | `tufa list/aid` in compatibility mode | Existing KLI store opens and identifiers are visible | `Harness-Ready` | `packages/keri/test/integration/app/interop-gates-harness.test.ts` |
