...
const VERSION_MODULE_TEMPLATE = await Deno.readTextFile(
  new URL("./templates/version-module.template.ts", import.meta.url),
);
...
function renderVersionModule(version: string, buildMetadata: string): string {
  ...
  return VERSION_MODULE_TEMPLATE
    .replace(JSON.stringify("__PACKAGE_VERSION__"), escapedVersion)
    .replace(JSON.stringify("__BUILD_METADATA__"), escapedMetadata);
}
