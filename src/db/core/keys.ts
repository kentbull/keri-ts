 ...
 */
export function dgKey(pre: string | Uint8Array, dig: string | Uint8Array): Uint8Array {
  const preBytes = typeof pre === 'string' ? new TextEncoder().encode(pre) : pre;
  const digBytes = typeof dig === 'string' ? new TextEncoder().encode(dig) : dig;
 ...
 */
export function onKey(pre: string | Uint8Array, sn: number, sep: string | Uint8Array = '.'): Uint8Array {
  const preBytes = typeof pre === 'string' ? new TextEncoder().encode(pre) : pre;
  const sepBytes = typeof sep === 'string' ? new TextEncoder().encode(sep) : sep;
  ...
  const result = new Uint8Array(preBytes.length + sepBytes.length + ordinalBytes.length);
  result.set(preBytes, 0);
 ...
 */
export function splitKey(key: Uint8Array | string, sep: string | Uint8Array = '.'): [Uint8Array, Uint8Array] {
  ...
  if (parts.length !== 2) {
    throw new Error(`Key must split into exactly 2 parts, got ${parts.length}`);
  }
 ...
 */
export function splitKeyON(key: Uint8Array | string, sep: string | Uint8Array = '.'): [Uint8Array, number] {
 ...
 */
export function suffix(key: Uint8Array | string, ion: number, sep: string | Uint8Array = '.'): Uint8Array {
  const keyBytes = typeof key === 'string' ? new TextEncoder().encode(key) : key;
  const sepBytes = typeof sep === 'string' ? new TextEncoder().encode(sep) : sep;
  ...
  const result = new Uint8Array(keyBytes.length + sepBytes.length + ordinalBytes.length);
 ...
 */
export function unsuffix(iokey: Uint8Array | string, sep: string | Uint8Array = '.'): [Uint8Array, number] {
  const iokeyBytes = typeof iokey === 'string' ? new TextEncoder().encode(iokey) : iokey;
  const sepBytes = typeof sep === 'string' ? new TextEncoder().encode(sep) : sep;
  ...
  if (lastSepIndex === -1) {
    throw new Error(`No separator found in iokey`);
  }
