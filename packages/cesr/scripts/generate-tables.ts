import {
  renderCanonicalCodexModule,
  renderCounterTables,
  renderIndexerTables,
  renderMatterTables,
  renderStringCodexes,
} from "./table-renderers.ts";
...
function parseNamedStringCodex(
  ...
  // Match KERIpy class attributes shaped like `Name: str = "code"` or
  // `Name: str = 'code'` inside a codex class body.
...
> {
  ...
  const chunk = text.slice(start, end);
  // Match Matter size rows shaped like `'code': Sizage(hs=..., ss=..., xs=...,
  // fs=None|number, ls=...)` and capture the code plus numeric fields.
...
> {
  ...
  const chunk = text.slice(start, end);
  // Match Indexer size rows shaped like `'code': Xizage(hs=..., ss=...,
  // os=..., fs=None|number, ls=...)` and capture the code plus numeric fields.
...
function parseCounterCodexes(
  ...
  // Match Counter codex class attributes shaped like `Name: str = 'code'` in
  // both versioned CounterCodex classes.
  const regex = /^\s+([A-Za-z0-9_]+):\s+str\s*=\s*'([^']+)'/gm;
...
function parseCounterSizes(
  ...
  // Match Counter size rows shaped like `'code': Cizage(hs=..., ss=..., fs=...)`
  // and capture the code plus numeric fields.
...
export async function buildGeneratedArtifacts(
  ...
  const artifacts = {
    "packages/cesr/src/tables/matter.tables.generated.ts": renderMatterTables(
      matterSizes,
      matterNames,
    ),
    "packages/cesr/src/tables/matter.codexes.generated.ts": renderStringCodexes(
      matterCodexes,
    ),
    "packages/cesr/src/tables/matter.codex.generated.ts": renderCanonicalCodexModule(
      canonicalMatterCodexes,
    ),
    "packages/cesr/src/tables/indexer.tables.generated.ts": renderIndexerTables(
      indexerSizes,
      indexerNames,
    ),
    "packages/cesr/src/tables/indexer.codexes.generated.ts": renderStringCodexes(
      indexerCodexes,
    ),
    "packages/cesr/src/tables/indexer.codex.generated.ts": renderCanonicalCodexModule(
      canonicalIndexerCodexes,
    ),
    "packages/cesr/src/tables/trait.codex.generated.ts": renderCanonicalCodexModule(
      canonicalTraitCodexes,
    ),
    "packages/cesr/src/tables/counter.tables.generated.ts": renderCounterTables(
      compatV1.sizes,
      compatV2Sizes,
      compatV1.names,
      counterNames.v2,
