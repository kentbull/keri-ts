...
async function formatTypeScript(source: string): Promise<string> {
  ...
  await writer.write(encoder.encode(source));
  await writer.close();

  const { code, stdout, stderr } = await child.output();
  if (code !== 0) {
    throw new Error(
      `deno fmt failed while formatting generated artifact: ${
...
function emitCanonicalCodexModule(
  ...
  const blocks = codexes.map(({ className, instanceName, entries }) => {
    const namedEntries = Object.entries(entries).sort(([a], [b]) =>
      a.localeCompare(b)
    );
...
export async function buildGeneratedArtifacts(
  ...
  const artifacts = {
    ...
    "packages/cesr/src/tables/matter.codex.generated.ts":
      emitCanonicalCodexModule(
    ...
    "packages/cesr/src/tables/indexer.codex.generated.ts":
      emitCanonicalCodexModule(
        canonicalIndexerCodexes,
      ),
    "packages/cesr/src/tables/trait.codex.generated.ts":
      emitCanonicalCodexModule(
