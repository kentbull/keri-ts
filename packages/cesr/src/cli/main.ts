...
const CESR_USAGE = [
  ...
  "",
  "Run `cesr <command> --help` for command-specific options.",
].join("\n");

/**
 * Execute the package-level `cesr` CLI dispatcher.
 *
 ...
 */
export async function cesrCli(args: string[], io: CliIo): Promise<number> {
  const [command, ...commandArgs] = args;
  if (command === "--help" || command === "-h") {
    await io.writeStdout(`${CESR_USAGE}\n`);
    return 0;
  }
  if (!command) {
    await io.writeStderr(`${CESR_USAGE}\n`);
    return 1;
  }

  const handler = COMMANDS[command];
  if (!handler) {
    await io.writeStderr(`Unknown cesr command: ${command}\n${CESR_USAGE}\n`);
    return 1;
  }
